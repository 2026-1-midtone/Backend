...
public record GetShiftsRequest(
        ...
        @Pattern(regexp = ValidationPatterns.DATE, message = "조회 시작일은 yyyy-MM-dd 형식이어야 합니다.")
        String from,
        @NotBlank(message = "조회 종료일은 필수 입력값입니다.")
        @Pattern(regexp = ValidationPatterns.DATE, message = "조회 종료일은 yyyy-MM-dd 형식이어야 합니다.")
        String to
) {}

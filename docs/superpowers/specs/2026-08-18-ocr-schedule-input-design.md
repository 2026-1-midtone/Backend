...
- 날짜 셀은 `yyyy-MM-dd`, `M/d`, `M월 d일`, 일(day) 숫자만 있는 형식을 지원한다. 일 숫자는 업로드 시 받은 `month`(yyyy-MM, 기본값 현재 달)와 결합한다. `month`는 재시도를 위해 잡에 저장한다.
- 시작·종료 시간은 null로 두고(반복 패턴 적용과 동일), 사용자가 초안 보정에서 입력할 수 있다.
- MVP는 "행마다 날짜 셀 + 근무 셀"인 표를 대상으로 한다. 달력 격자형 근무표는 이후 확장.

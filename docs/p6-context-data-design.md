...
채팅 모델 공급자는 `ChatAnswerGenerator` 포트 뒤에서 교체한다. `GEMINI_API_KEY`가 설정되면 Gemini(`generateContent` + responseSchema 구조화 출력, 기본 `gemini-3.5-flash-lite`)를, 없으면 OpenAI를 사용한다. 두 공급자는 동일한 시스템 프롬프트와 입력 조립(`ChatPromptTexts`)을 공유한다. OCR 폴백은 공급자 선택과 무관하게 `OPENAI_API_KEY`를 따로 사용한다.

...
- **들여쓰기(중첩)는 2단계를 넘기지 않는다.** if/for가 겹겹이 쌓이면 early return이나 메서드 추출로 평탄화한다.
  ...
  ...
  ```
- 매개변수가 많아지면(대략 4개 초과) 우선 DTO/record로 묶을 수 있는지 본다. 묶이지 않는 서로 다른 협력 객체들이라면, 파라미터 개수 자체보다 "그 협력 객체들이 각자 독립적으로 존재할 이유가 있는가"를 기준으로 판단한다.
  - 재사용되거나(예: `TransitionDetector`는 coaching·transition 양쪽에서 씀), 이름만으로 책임이 분명하거나, 독립적으로 테스트할 가치가 있는 협력 객체는 파라미터 개수와 무관하게 별도 클래스로 분리한다.
  - 반대로 서로 다른 도메인의 협력 객체를 "파라미터 개수를 줄이기 위해서만" 하나의 클래스로 묶지 않는다. 억지로 쪼개거나 억지로 묶는 것 둘 다 피한다 — 특히 홈 대시보드처럼 여러 도메인을 그러모아 응답을 구성하는 파사드성 서비스는 협력 객체가 자연스럽게 5개, 6개가 될 수 있고, 그 자체는 문제가 아니다.
  - 다만 한 클래스가 의존하는 협력 객체가 과도하게 많아지면(대략 7~8개 이상) 여러 책임이 뒤섞여 있다는 신호일 수 있으니 도메인 경계를 다시 검토한다.
